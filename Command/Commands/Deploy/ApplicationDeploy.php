...
//      ApplicationInitalizeInputData();
        ...
        */
        ...
        $environment = $this->ask("Please select the environment you wish to reflect. [".implode(',',array_keys($environments['deploy'])) . "] : " , false);
    ...
    private function createZip($environment)
    {
        ...
        $isGit = $this->ask("Enter Yes if you want to reflect some of them using Git commands. [yes or no]: " , false);
        
        if($isGit === 'yes')
        {
            exec("git add -N .; git diff --name-only --relative=src/ | xargs -I % cp -r --parents ./src/% .tmp/$environment");
