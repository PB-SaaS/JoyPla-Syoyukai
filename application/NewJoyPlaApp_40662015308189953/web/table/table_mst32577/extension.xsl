     ...
     <thead>
      <tr>
        ...
        <th>
          <a href="{/table/fieldList/field[@title='registrationTime']/@sort}">
            ...
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349775'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='updateTime']/@sort}">
            ...
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349776'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='distributorId']/@sort}">
            <xsl:text>卸業者ID</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002348841'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='distributorName']/@sort}">
            <xsl:text>卸業者名</xsl:text>
            <xsl:call-template name="sortText">
      ...
      <xsl:for-each select="data/record">
        ...
        <tr style="height:20;">
          ...
          <td>
            <xsl:value-of select="usr_distributorId" />
          </td>
          <td>
            <xsl:value-of select="usr_distributorName" />
          </td>
          <td>
            <a class="uk-button uk-button-primary" href="{/table/cardList/card[@title='page_265119'][@recordId=$id]}" target="_self">
            詳細
            </a>
          </td>
        </tr>
