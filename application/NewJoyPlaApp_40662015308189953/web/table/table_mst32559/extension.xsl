     ...
     <thead>
        ...
        <th>
          <a href="{/table/fieldList/field[@title='registrationTime']/@sort}">
            <xsl:text>棚卸日時</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349691'" />
            </xsl:call-template>
          </a>
        </th>
        <th class="no_fix">
          <a href="{/table/fieldList/field[@title='inventoryHId']/@sort}">
            <xsl:text>棚卸履歴番号</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002348932'" />
            </xsl:call-template>
          </a>
        </th>
        <th class="fix">
        ...
        <th>
          <a href="{/table/fieldList/field[@title='itemsNumber']/@sort}">
            ...
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349693'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='totalAmount']/@sort}">
            ...
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349694'" />
            </xsl:call-template>
          </a>
        </th>
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          ...
          <td>
            <xsl:value-of select="usr_inventoryHId" />
          </td>
          <td>
          ...
          <td><xsl:if test="usr_totalAmount != ''">￥<xsl:value-of select="format-number(usr_totalAmount, '#,###.00')" /></xsl:if></td>
          <td class="fix">
            <a class="uk-button uk-button-primary" href="{/table/cardList/card[@title='page_263685'][@recordId=$id]}" target="_self">
            詳細
            </a>
          </td>
          <td class="no_fix">
            <a class="uk-button uk-button-primary" href="{/table/cardList/card[@title='page_263646'][@recordId=$id]}" target="_self">
            詳細
            </a>
          </td>
        </tr>
