     ...
     <thead>
      <tr>
        ...
        <th>
          <a href="{/table/fieldList/field[@title='registrationTime']/@sort}">
            <xsl:text>返品日時</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349657'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
        ...
        <th>
          <a href="{/table/fieldList/field[@title='returnHistoryID']/@sort}">
            <xsl:text>返品番号</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002348891'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='itemsNumber']/@sort}">
            <xsl:text>返品品数</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349659'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
          <a href="{/table/fieldList/field[@title='returnTotalPrice']/@sort}">
            <xsl:text>返品合計金額</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349661'" />
            </xsl:call-template>
          </a>
        </th>
        <th class="uk-table-shrink">
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          ...
          <td>
            <xsl:value-of select="usr_returnHistoryID" />
          </td>
          <td>
            <xsl:value-of select="usr_itemsNumber" />
          </td>
          <td>
            <xsl:if test="usr_returnTotalPrice">￥-<xsl:value-of select="format-number(usr_returnTotalPrice,'#,###.00')" />
			</xsl:if>
          </td>
          <td>
            <a class="uk-button uk-button-primary" href="{/table/cardList/card[@title='page_265604'][@recordId=$id]}" target="_self">
              詳細
            </a>
          </td>
        </tr>
