      ...
      <tr class="smp-row-3 smp-row-sort" style="height:20;">
        ...
        <th>
          <a href="{/table/fieldList/field[@title='registrationTime']/@sort}">
            <xsl:text>未発注書作成日時</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="'f002349740'" />
            </xsl:call-template>
          </a>
        </th>
        <th>
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          ...
          <td>
            ...
            <xsl:if test="$totalAmount">￥<xsl:value-of select="format-number($totalAmount,'#,###.00')" />
			</xsl:if>
          </td>
          <td>
          	<span class="uk-label uk-padding-small uk-label-danger uk-padding-remove-vertical">
            <xsl:value-of select="usr_orderStatus" />
            </span>
          </td>
          <td>
            <a href="{/table/cardList/card[@title='page_262926'][@recordId=$id]}" target="_self" class="uk-button uk-button-primary">
	            詳細
            </a>
          </td>
        </tr>
