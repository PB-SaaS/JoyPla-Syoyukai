          ...
          <tr>
            ...
            <th>
              <a href="{/table/fieldList/field[@title='registrationTime']/@sort}">
                <xsl:text>物品請求日時</xsl:text>
                <xsl:call-template name="sortText">
                  <xsl:with-param name="field" select="'f002349756'" />
                </xsl:call-template>
              </a>
            </th>
            <th>
              <a href="{/table/fieldList/field[@title='billingNumber']/@sort}">
                <xsl:text>物品請求番号</xsl:text>
                <xsl:call-template name="sortText">
                  <xsl:with-param name="field" select="'f002349000'" />
                </xsl:call-template>
              </a>
            </th>
            <th>
              <a href="{/table/fieldList/field[@title='divisionName']/@sort}">
                <xsl:text>請求部署名</xsl:text>
                <xsl:call-template name="sortText">
            ...
            <th>
              <a href="{/table/fieldList/field[@title='itemsNumber']/@sort}">
                ...
                <xsl:call-template name="sortText">
                  <xsl:with-param name="field" select="'f002349758'" />
                </xsl:call-template>
              </a>
            </th>
            <th>
              <a href="{/table/fieldList/field[@title='totalAmount']/@sort}">
                ...
                <xsl:call-template name="sortText">
                  <xsl:with-param name="field" select="'f002349759'" />
                </xsl:call-template>
              </a>
            </th>
        ...
        <tbody>
          <xsl:for-each select="data/record">
            ...
            <tr>
              ...
              <td>
                  <xsl:value-of select="usr_billingNumber" />
              </td>
              <td>
              ...
              <td>
                <a href="{/table/cardList/card[@title='page_263400'][@recordId=$id]}" target="_self" class="uk-button uk-button-primary">
                  詳細
                </a>
              </td>
            </tr>
