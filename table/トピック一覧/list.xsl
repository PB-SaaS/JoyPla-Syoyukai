  ...
  <form method="post" action="{@action}">
    ...
    <table class="uk-table uk-table-hover uk-table-middle uk-table-divider uk-text-nowrap">
     <thead>
      <tr>
        <th class="">
          <a href="{/table/fieldList/field[@title='topicTitle']/@sort}">
            <xsl:text>タイトル</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="/table/fieldList/field[@title='topicTitle']/@code" />
            </xsl:call-template>
          </a>
        </th>
        <th class="">
          <a href="{/table/fieldList/field[@title='commentCount']/@sort}">
            <xsl:text>コメント数</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="/table/fieldList/field[@title='commentCount']/@code" />
            </xsl:call-template>
          </a>
        </th>
        <th class="">
          <a href="{/table/fieldList/field[@title='topicName']/@sort}">
            <xsl:text>作成者</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="/table/fieldList/field[@title='topicName']/@code" />
            </xsl:call-template>
          </a>
        </th>
        <th class="">
          <a href="{/table/fieldList/field[@title='lastName']/@sort}">
            <xsl:text>最終更新者</xsl:text>
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="/table/fieldList/field[@title='lastName']/@code" />
            </xsl:call-template>
          </a>
        </th>
        <th class="uk-width-medium ">
          <a href="{/table/fieldList/field[@title='updateTime']/@sort}">
            <xsl:text>最終更新日時</xsl:text>
            <xsl:call-template name="sortText">
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          <td>
            <a class="uk-button uk-button-link" href="{/table/cardList/card[@title='page_266279'][@recordId=$id]}" target="_self">
              <xsl:value-of select="usr_topicTitle" />
            </a>
          </td>
          <td>
            <xsl:value-of select="usr_commentCount" />件
          </td>
          <td>
            <xsl:value-of select="usr_topicName" />
          </td>
          <td>
            <xsl:value-of select="usr_lastName" />
          </td>
          <td>
            <span class="uk-margin-small-right"><xsl:value-of select="usr_updateTime/year" />/<xsl:value-of select="format-number(usr_updateTime/month,'00')" />/<xsl:value-of select="format-number(usr_updateTime/day,'00')" /></span>
            <span><xsl:value-of select="format-number(usr_updateTime/hour,'00')" />:<xsl:value-of select="format-number(usr_updateTime/minute,'00')" /></span>
          </td>
        </tr>
      </xsl:for-each>
      </tbody>
