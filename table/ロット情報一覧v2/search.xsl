  ...
  <div>
    <form method="get">
      ...
      <div class="uk-width-3-4@m uk-margin-auto">
        ...
        <xsl:apply-templates select="fieldList/usr_lotNumber" />
        <xsl:apply-templates select="fieldList/usr_lotDate" />
...
<!-- ロット番号 -->
<xsl:template match="usr_lotNumber">
  <div class="uk-margin">
...
<!-- 使用期限 -->
<xsl:template match="usr_lotDate">
  <div class="uk-margin">
    <label class="uk-form-label" for="form-stacked-text">使用期限</label>
    <input type="hidden" name="{option/exType/@name}" value="16" />
    <div class="uk-form-controls uk-child-width-1-2@m uk-grid">
      <div class="uk-margin-small-bottom">
      <input class="uk-input uk-width-3-4 uk-margin-small-right" type="date" name="{main/value1/@name}">
