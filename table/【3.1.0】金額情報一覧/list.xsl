...
<xsl:template match="/table">
  ...
  <form method="post" action="{@action}">
    ...
    <div class="">
     ...
     <input type="button" id="exportButton" value="ダウンロード" class="smp-table-button with-wrap uk-hidden" onclick="SpiralTable.setDLFileName(this, {data/@limit});" />
    ...
    <table class="uk-table uk-table-striped uk-text-nowrap ">
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          ...
          <td>
            <a class="smp-cell-id" href="{/table/cardList/card[@title='page_169005'][@recordId=$id]}" target="_blank">
              詳細
            </a>
          </td>
          <td>
          ...
          <td>
            ...
            <xsl:value-of select="usr_quantityUnit" /> / <xsl:value-of select="usr_itemUnit" />
          </td>
          <td>
