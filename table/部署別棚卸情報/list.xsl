     ...
     <thead>
        ...
        <th class="no_fix">
          <a href="{/table/fieldList/field[@title='inventoryHId']/@sort}">
            ...
            <xsl:call-template name="sortText">
              <xsl:with-param name="field" select="/table/fieldList/field[@title='inventoryHId']/@code" />
            </xsl:call-template>
          </a>
        </th>
        <th class="fix">
      ...
      <xsl:for-each select="data/record">
        ...
        <tr>
          ...
          <td><xsl:if test="usr_totalAmount != ''">￥<xsl:value-of select="format-number(usr_totalAmount, '#,##0.00')" /></xsl:if></td>
          <td class="fix">
