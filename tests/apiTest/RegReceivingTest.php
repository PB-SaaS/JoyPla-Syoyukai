<?php 

namespace phpUnit\Test;

use PHPUnit\Framework\TestCase; 

require_once ('src/NewJoyPla/lib/ApiSpiral.php');
require_once ('src/NewJoyPla/lib/Define.php');
require_once ('src/NewJoyPla/lib/UserInfo.php');
require_once ('src/NewJoyPla/lib/SpiralDataBase.php');
require_once ('mock/Spiral.php');
require_once ('mock/SpiralApiRequest.php');
require_once ('mock/PbSpiralApiCommunicator.php');
require_once ('src/NewJoyPla/api/RegReceiving.php');


class RegReceivingTest extends TestCase
{
    private $Spiral;

    private $returnSuccess =  array(
        'data' => array(),
        'code' => '0',
        'message' => 'message',
        'count' => '1'
    );

    private $returnError =  array(
        'data' => array(),
        'code' => '1',
        'message' => 'message',
        'count' => '0'
        );
    
        
    public function testRegister()
    {
        $spiral = new \Spiral();
         
        $userInfo = new \App\Lib\UserInfo($spiral);
        $spiralApiCommunicator = $spiral->getSpiralApiCommunicator();
        
        //Success
        $spiralApiRequest = $this->createMock(\SpiralApiRequest::class);
        $spiralApiRequest->method('entrySet')->willReturn($this->returnSuccess);
        $spiralDataBase = new \App\Lib\SpiralDataBase($spiral,$spiralApiCommunicator,$spiralApiRequest);
        
        $regReceiving = new \App\Api\RegReceiving($spiralDataBase,$userInfo);

        $array = array(
            'inHpItemsId' => array(
                'orderCNumber' => 'test00001',
                'price' => '10',
                'orderQuantity' => '1',//注文数
                'receivingNowCount' => '1',//前回までの入庫数
                'receivingCount' => '10'
            )
        );

        //$data['orderQuantity'] - ( (int)$data['receivingNowCount']  + (int)$data['receivingCount'] ) <= 0


        $this->assertTrue($regReceiving->register('orderHistoryId','distributorId','divisionId',$array));

        
        //Success
        $spiralApiRequest = $this->createMock(\SpiralApiRequest::class);
        $spiralApiRequest->method('entrySet')->willReturn($this->returnSuccess);
        $spiralDataBase = new \App\Lib\SpiralDataBase($spiral,$spiralApiCommunicator,$spiralApiRequest);
        
        $regReceiving = new \App\Api\RegReceiving($spiralDataBase,$userInfo);

        $array = array(
            'inHpItemsId' => array(
                'orderCNumber' => 'test00001',
                'price' => '10',
                'orderQuantity' => '100',//注文数
                'receivingNowCount' => '1',//前回までの入庫数
                'receivingCount' => '10'
            )
        );

        //$data['orderQuantity'] - ( (int)$data['receivingNowCount']  + (int)$data['receivingCount'] ) <= 0
        $this->assertTrue($regReceiving->register('orderHistoryId','distributorId','divisionId',$array));

        
        
        //Error
        $spiralApiRequest = $this->createMock(\SpiralApiRequest::class);
        $spiralApiRequest->method('entrySet')->willReturn($this->returnError);
        $spiralDataBase = new \App\Lib\SpiralDataBase($spiral,$spiralApiCommunicator,$spiralApiRequest);
        
        $regReceiving = new \App\Api\RegReceiving($spiralDataBase,$userInfo);

        $array = array(
            'inHpItemsId' => array(
                'orderCNumber' => 'test00001',
                'price' => '10',
                'orderQuantity' => '100',//注文数
                'receivingNowCount' => '1',//前回までの入庫数
                'receivingCount' => '10'
            )
        );

        //$data['orderQuantity'] - ( (int)$data['receivingNowCount']  + (int)$data['receivingCount'] ) <= 0
        $this->assertFalse($regReceiving->register('orderHistoryId','distributorId','divisionId',$array));

    }
}