...
class BarcodeController extends Controller
{
    public function search($vars , BarcodeSearchInputPortInterface $inputPort ) 
    {
        ...
        $inputData = new BarcodeSearchInputData($this->request->user(),$barcode);
        $inputPort->handle($inputData);
    }

    public function orderSearch($vars , BarcodeOrderSearchInputPortInterface $inputPort ) 
    {
        $token = $this->request->get('_csrf');
        Csrf::validate($token,true);
        ...
        $inputData = new BarcodeOrderSearchInputData($this->request->user(),$barcode , $gate->isOnlyMyDivision());
        $inputPort->handle($inputData);
    }
