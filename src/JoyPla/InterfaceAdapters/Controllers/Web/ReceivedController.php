...
class ReceivedController extends Controller
{
    ...
    public function orderReceivedSlipIndex($vars, OrderReceivedSlipIndexInputPortInterface $inputPort ) 
    {
        ...
        $inputData = new OrderReceivedSlipIndexInputData($this->request->user(),$vars['orderId'],$gate->isOnlyMyDivision());
    ...
    public function index($vars, ReceivedIndexInputPortInterface $inputPort ) 
    {
        ...
        $inputData = new ReceivedIndexInputData($this->request->user(),$vars['receivedId'],$gate->isOnlyMyDivision());
        $inputPort->handle($inputData);
    }
    
    public function labelsetting($vars, ReceivedIndexInputPortInterface $inputPort ) 
    {
        if(Gate::denies('list_of_acceptance_inspection_slips'))
        {
            Router::abort(403); 
