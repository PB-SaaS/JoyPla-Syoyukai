 ...
 */
    ...
    public function register($vars)
    {
        ...
        $tablecardUrl = $this->request->get('tablecardUrl');
        if ($this->request->get('tablecardUrl')) {
            $this->request
                ->session()
                ->put('tablecardUrl', $this->request->get('tablecardUrl'));
        }
        if ($this->request->confirm) {
            $tablecardUrl = $this->request->session()->get('tablecardUrl');
            $sessionItemData = $this->request->session()->get($this->itemData);
            $itemId = $sessionItemData['itemId']
                ? $sessionItemData['itemId']
                : $this->request->get('itemId');
            $validate = SiValidator::make($input, $this->rules, $this->labels);
        }
        $data = SpiralDB::title('NJ_itemDB')
            ->value([
                'itemId',
                'tenantId',
                'makerName',
        ...
        if (
            ...
            $user->userPermission ==
                '3' /*  && $item->tenantId === $this->request->user()->tenantId */
        ) {
            $body = view('html/Product/PriceAndInHospitalItemRegist/input', [
                'distributor' => $distributor,
                'input' => $this->request->all(),
                'item' => $item,
                'tablecardUrl' => $tablecardUrl,
    ...
    public function confirm(array $vars)
    {
        ...
        if ($validate->isError()) {
            ...
            Router::redirect(
                '/product/PriceAndInHospitalRegist/input',
                $this->request
            );
            exit();
        }
    ...
    public function thanks(array $vars)
    {
        ...
        $body = view('html/Product/PriceAndInHospitalItemRegist/thanks', [
            'result' => $result,
            'tablecardUrl' => $tablecardUrl,
        ]);
        echo view('html/Common/Template', compact('body'), false);
