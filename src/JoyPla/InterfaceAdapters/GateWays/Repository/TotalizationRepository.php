    ...
    public function search(HospitalId $hospitalId, object $search)
    {
        ...
        if (is_array($search->sourceDivisionIds) && count($search->sourceDivisionIds) > 0) {
            foreach ($search->sourceDivisionIds as $sourceDivisionId) {
                $totalzationByDivisionInstance->orWhere('sourceDivisionId', $sourceDivisionId);
            }
            $byDivisionSearchFlag = true;
        }

        if ($byDivisionSearchFlag) {
            $totalzationByDivisions = $totalzationByDivisionInstance->sort('id', 'asc')->get();
        ...
        if ($search->itemName) {
            $totalzationByInHPItemInstance->orWhere('itemName', "%" . $search->itemName . "%", "LIKE");
        }
        if ($search->makerName) {
            $totalzationByInHPItemInstance->orWhere('makerName', "%" . $search->makerName . "%", "LIKE");
        }
        if ($search->itemCode) {
            $totalzationByInHPItemInstance->orWhere('itemCode', "%" . $search->itemCode . "%", "LIKE");
        }
        if ($search->itemStandard) {
            $totalzationByInHPItemInstance->orWhere('itemStandard', "%" . $search->itemStandard . "%", "LIKE");
        }
        if ($search->itemJANCode) {
            $totalzationByInHPItemInstance->orWhere('itemJANCode', "%" . $search->itemJANCode . "%", "LIKE");
        }
        if (is_array($search->targetDivisionIds) && count($search->targetDivisionIds) > 0) {
            foreach ($search->targetDivisionIds as $targetDivisionId) {
                $totalzationByInHPItemInstance->orWhere('targetDivisionId', $targetDivisionId);
            }
        }

        $totalzationByInHPItems = $totalzationByInHPItemInstance->sort('id', 'asc')->page($search->currentPage)->paginate($search->perPage);
        ...
        if ($byDivisionSearchFlag) {
            ...
            foreach ($totalzationByInHPItems->data->all() as $totalzationByInHPItem) {
                ...
                foreach ($totalzationByDivisions->data->all() as $totalzationByDivision) {
                    if (($totalRequestItem->getRecordId()) === ($totalzationByDivision->recordId)) {
                        $source_division_find_key = array_search($totalzationByDivision->sourceDivisionId, collect_column($division, 'divisionId'));
                        $target_division_find_key = array_search($totalzationByDivision->targetDivisionId, collect_column($division, 'divisionId'));
                        ...
                        $totalRequestItem = $totalRequestItem->addTotalRequest(TotalRequest::create($totalzationByDivision));
                    }
                }
        ...
        /*  請求元指定なし  */
        foreach ($totalzationByInHPItems->data->all() as $totalzationByInHPItem) {
            $totalzationByDivisionInstance->orWhere('recordId', $totalzationByInHPItem->recordId);
        }
        ...
        foreach ($totalzationByInHPItems->data->all() as $totalzationByInHPItem) {
            ...
            foreach ($totalzationByDivisions->data->all() as $totalzationByDivision) {
                if (($totalRequestItem->getRecordId()) === ($totalzationByDivision->recordId)) {
                    $source_division_find_key = array_search($totalzationByDivision->sourceDivisionId, collect_column($division, 'divisionId'));
                    $target_division_find_key = array_search($totalzationByDivision->targetDivisionId, collect_column($division, 'divisionId'));
                    if (($source_division_find_key !== false) && ($target_division_find_key !== false)) {
                        $sourceDivision = new Collection();
                        $sourceDivision->hospitalId = $hospitalId->value();
                        $sourceDivision->divisionId = $totalzationByDivision->sourceDivisionId;
                        $sourceDivision->divisionName = htmlspecialchars_decode($division[$source_division_find_key]->divisionName, ENT_QUOTES);
