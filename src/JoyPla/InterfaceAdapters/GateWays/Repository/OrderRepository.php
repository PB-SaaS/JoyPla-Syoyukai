    ...
    OrderRepositoryInterface
{
    ...
    public function searchItems(HospitalId $hospitalId, object $search)
    {
        $itemViewInstance = ModelRepository::getOrderItemsView()
            ->where('hospitalId', $hospitalId->value())
            ->value('orderNumber')
            ->value('orderCNumber');
        ...
        if ($search->smallCategory) {
            $itemViewInstance->orWhere(
                'smallCategory',
                '%' . $search->smallCategory . '%',
                'LIKE'
            );
        }
        if ($search->catalogNo) {
        ...
        if (
            ...
            foreach ($search->distributorIds as $distributorId) {
                $itemViewInstance->orWhere('distributorId', $distributorId);
            }
        }

        if (is_array($search->divisionIds) && count($search->divisionIds) > 0) {
            foreach ($search->divisionIds as $divisionId) {
                $itemViewInstance->orWhere('divisionId', $divisionId);
            }
        }

        if (is_array($search->orderStatus) && count($search->orderStatus) > 0) {
            foreach ($search->orderStatus as $orderStatus) {
                $itemViewInstance->orWhere('orderStatus', $orderStatus);
            }
        }
        ...
        return [$items->getData()->all(), $items->getTotal()];
    }
}
