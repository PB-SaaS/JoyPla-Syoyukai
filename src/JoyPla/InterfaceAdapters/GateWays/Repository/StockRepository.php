...
class StockRepository implements StockRepositoryInterface
{
    public function search(Auth $auth , object $search)
    {
        ...
        $inHospitalItemIds = InHospitalItemView::where('notUsedFlag','1','!=')->where('hospitalId',$auth->hospitalId)->value('inHospitalItemId');

        if($search->itemName !== "")
        {
            $inHospitalItemIds->orWhere('itemName',"%".$search->itemName."%","LIKE");
        }
        if($search->makerName !== "")
        {
            $inHospitalItemIds->orWhere('makerName',"%".$search->makerName."%","LIKE");
        }
        if($search->itemCode !== "")
        {
            $inHospitalItemIds->orWhere('itemCode',"%".$search->itemCode."%","LIKE");
        }
        if($search->itemStandard !== "")
        {
            $inHospitalItemIds->orWhere('itemStandard',"%".$search->itemStandard."%","LIKE");
        }
        if($search->itemJANCode !== "")
        {
            $inHospitalItemIds->orWhere('itemJANCode',"%".$search->itemJANCode."%","LIKE");
        }
        ...
        {
            $stocks->orWhere('inHospitalItemId',$inHospitalItemid->inHospitalItemId);
        }
        
        $stocks = $stocks->sort('id','desc')->page($search->currentPage)->paginate($search->perPage);
        ...
        $inHospitalItem = InHospitalItemView::getNewInstance()->where('hospitalId',$auth->hospitalId);
