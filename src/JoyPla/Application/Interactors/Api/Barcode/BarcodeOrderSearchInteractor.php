...
namespace JoyPla\Application\Interactors\Api\Barcode {
     ...
     */
    class BarcodeOrderSearchInteractor implements BarcodeOrderSearchInputPortInterface
    {
        /**
         * BarcodeOrderSearchInteractor constructor.
         * @param BarcodeOrderSearchOutputPortInterface $outputPort
         */
        public function __construct(
            BarcodeOrderSearchOutputPortInterface $outputPort ,
        ...
        public function handle(BarcodeOrderSearchInputData $inputData)
        {
            ...
            if((preg_match('/^1/', $inputData->barcode) && strlen($inputData->barcode) == 14 ) || (preg_match('/^01/', $inputData->barcode) && strlen($inputData->barcode) == 14)){
                ...
                [ $orders , $count ] = $this->barcodeRepository->orderSearchByInHospitalItemId((new HospitalId($inputData->user->hospitalId)) ,(new InHospitalItemId($inHospitalItems->inHospitalItemId)) , $divisionId);
                ...
                [ $orders , $count ] = $this->barcodeRepository->orderSearchByJanCode((new HospitalId($inputData->user->hospitalId)) , (string)$inputData->barcode , $divisionId);
...
namespace JoyPla\Application\InputPorts\Api\Barcode {
    ...
    {
        /**
         * BarcodeOrderSearchInputData constructor.
         */
        public function __construct(Auth $user , string $barcode , bool $isOnlyMyDivision)
        {
            $this->user = $user;
