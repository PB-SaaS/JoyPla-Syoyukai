  ...
  <div id="content" class="flex h-full px-1">
    <div class="flex-auto">
      <div class="index container mx-auto">
        <h1 class="text-2xl mb-2">返品書一覧</h1>
        <hr>
        ...
        <div class="pt-2 hover:bg-sushi-50" v-for="(receivedReturn) in receivedReturns">
          <div class="border-b-2 border-solid border-gray-100 w-full">
            <div class="lg:flex lg:divide-x ">
              <div class="lg:w-1/5 p-2">
                <p class="text-md font-bold">登録日時<br>{{ receivedReturn.registDate }}</p>
                <p class="text-md">
                  発注番号：{{ receivedReturn.orderId }}<br>
              ...
              <div class="lg:w-4/5 p-2">
                <div class="w-full lg:flex mt-3" v-for="(receivedReturnItem) in receivedReturn.returnItems">
                  <div class="lg:flex-1 flex lg:w-3/4">
                    <item-view class="md:h-44 md:w-44 h-32 w-32" :base64="receivedReturnItem.itemImage"></item-view>
                    <div class="flex-1 pl-4 lg:flex gap-6 break-all">
                      <div class="flex-auto lg:w-4/5 w-full">
                        <h3 class="text-xl font-bold font-heading">{{ receivedReturnItem.item.makerName }}</h3>
                        <p class="text-md font-bold font-heading">{{ receivedReturnItem.item.itemName }}</p>
                        <p class="text-md text-gray-500">{{ receivedReturnItem.item.itemCode }}</p>
                        <p class="text-md text-gray-500">{{ receivedReturnItem.item.itemStandard }}</p>
                        <p class="text-md text-gray-500">{{ receivedReturnItem.item.itemJANCode }}</p>
                        <p class="text-md text-gray-900" v-if="( receivedReturnItem.lot.lotNumber != '' && receivedReturnItem.lot.lotDate != '' )">
                        ロット情報：{{ receivedReturnItem.lot.lotNumber }} / {{ receivedReturnItem.lot.lotDate }} 
                        </p>
                        <p class="text-base text-gray-900">
                        {{ numberFormat(receivedReturnItem.returnQuantity) }}{{ receivedReturnItem.quantity.itemUnit }}
                        </p>
                        <p>
                          <span class="text-blue-700 text-lg mr-4">&yen; {{ numberFormat(receivedReturnItem.returnPrice) }}</span>
                          <span class="text-sm text-gray-900">( &yen; {{ numberFormat(receivedReturnItem.price) }} / {{ receivedReturnItem.quantity.itemUnit }} )</span>
                        </p>
                      </div>
                    </div>
                  </div>
...
var JoyPlaApp = Vue.createApp({
    ...
    setup(){
      ...
      const pagetitle = "receivedReturnshow";
      ...
      const setParam = (values) => {
        ...
        url.searchParams.set('receivedReturnDate',values.receivedReturnDate);
      ...
      const listGet = () => {
        ...
        params.append("path", "/api/return/show");
        ...
        .then( (response) => {
          receivedReturns.value = response.data.data;
