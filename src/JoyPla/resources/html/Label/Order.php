    ...
    <div id="content" class="flex h-full px-1">
        <div class="flex-auto">
            <div class="index container mx-auto no_print">
                <h1 class="text-2xl my-2">発注商品ラベル発行</h1>
                <hr>
                <div class="my-4">
                    <table class="table-auto w-full text-sm">
                        <thead>
                            <tr>
                                <th class="border border-slate-100 font-medium p-4 pr-8 text-left w-1/3">商品情報</th>
<!-- 
 ...
 -->
                                <th class="border border-slate-100 font-medium p-4 pr-8 text-left w-1/3">数量</th>
                                <th class="border border-slate-100 font-medium p-4 pr-8 text-left w-1/3">枚数</th>
                            </tr>
                        </thead>
                        <tbody>
                                    ...
                                    <template v-for="(print , printIdx) in order.print">
                                        <tr>
                                            <td v-if="pIdx === 0 && printIdx === 0" class="border border-slate-100 p-4 pr-8 text-slate-500" :rowspan="calcRow(inHospitalItem.order)">
                                                <div>
                                                    <p>{{ inHospitalItem.divisionName }}</p>
                                                    <p>{{ inHospitalItem.distributorName }}</p>
                                                    <p>{{ inHospitalItem.makerName }}</p>
                                                    <p>{{ inHospitalItem.itemName }}</p>
                                                    <p>{{ inHospitalItem.itemCode }}</p>
                                                    <p>{{ inHospitalItem.itemStandard }}</p>
                                                    <p>{{ inHospitalItem.itemJANCode }}</p>
                                                    <p>{{ inHospitalItem.price }}円 / {{ inHospitalItem.quantity }}{{ inHospitalItem.quantityUnit }}</p>
                                                </div>
                                            </td>
<!-- 
 ...
 -->
                                            <td class="border border-slate-100 p-4 pr-8 text-slate-500">
                                                {{values.items[idx].order[pIdx].print[printIdx].count}}{{inHospitalItem.quantityUnit}}
<!-- 
                                            <v-input-number 
                                                :name="`items[${idx}].order[${pIdx}].print[${printIdx}].count`"
    ...
    var JoyPlaApp = Vue.createApp({
        ...
        setup() {
            ...
            onMounted( () => {
                ...
                {
                    JsBarcode("#"+idname,value,{width: 1.8, height: values.setting.barcodeHeight, fontSize: 14, format: "CODE128"});
                }
            ...
            const createLabelModel = (values) => {
                ...
                values.items.forEach(item => {
                    item.order.forEach(order => {
                        let obj = {};
                        obj.orderCNumber = order.orderCNumber
            ...
            const labelReload = handleSubmit(async (values) => {
                ...
                params.path = "/label/order/" + orderNumber;
                params._method = 'get';
                params._csrf = _CSRF;
            ...
            const copy = (idx,pIdx) => {
                values.items[idx].order[pIdx].print.push({
                    count: 0 ,
                    print: 0 ,
                    isDeleteButton : true
                });
            ...
            const removeItem = (idx,pIdx, printIdx) => {
                values.items[idx].order[pIdx].print.splice( printIdx, 1 );
                replace(values.items)
            }
