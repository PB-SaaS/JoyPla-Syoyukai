  ...
  <div id="content" class="flex h-full px-1">
    <div class="flex-auto">
      <div class="index container mx-auto mb-96">
        <h1 class="text-2xl mb-2">一括編集</h1>
        <hr>
        <div class="my-2" v-for="(item, idx) in fields" :key="item.key">
          <div class="w-full lg:flex mt-3">
            <div class="flex-auto lg:flex-1 flex lg:w-3/4">
              <item-view class="md:h-44 md:w-44 h-32 w-32" :base64="item.value.item.inItemImage"></item-view>
              <div class="flex-1 px-4 relative">
                <div class="flex-auto lg:flex justify-between leading-normal lg:space-y-0 space-y-4 gap-6">
                  <div class="w-1/3">
                    <p class="text-md font-bold" v-if="item.value.orderId">発注伝票番号: {{ item.value.orderId }}</p>
                    <p class="text-md font-bold" v-if="item.value.orderItemId">発注番号: {{ item.value.orderItemId }}</p>
                    <p class="text-md font-bold" v-if="item.value.division.divisionName">発注元部署: {{ item.value.division.divisionName }}</p>
                  </div>
                  <div class="w-2/3">
                    ...
                    <p class="text-gray-500" v-if="item.value.item.itemJANCode">{{ item.value.item.itemJANCode }}</p>
            ...
            <div class="flex-initial lg:flex gap-6 lg:w-1/4 items-end flex-row-reverse">
              <v-input-number :rules="{ between: [-99999 , 99999] }" :name="`orderItems[${idx}].orderQuantity`" label="発注数（個数）" :unit="item.value.quantity.itemUnit" :step="1" :title="`発注数（個数）/${item.value.quantity.quantityNum}${ item.value.quantity.quantityUnit }入り`"></v-input-number>
            </div>
          </div>
        </div>
      </div>
