  ...
  <div id="content" class="flex h-full px-1">
    <div class="flex-auto">
      <div class="index container mx-auto mb-96">
        ...
        <hr>
        <div class="py-5">
          <v-button-primary type="button" :disabled="! isChange" @click.native="onUpdate">内容を更新</v-button-primary>
        </div>
    ...
    setup(){
      ...
      const breadcrumbs = [
          ...
          },
          {
            text: '未発注一覧',
            disabled: false,
            href: '%url/rel:mpgt:Root%&path=/order/unapproved/show&isCache=true',
          },
          {
            text: '未発注伝票の詳細',
            disabled: true,
          ...
          Swal.fire({
            ...
            if(result.isConfirmed){
              ...
              Swal.fire({
                ...
                if(res.data.data.isOrderDeleted)
                {
                  location.href = "%url/rel:mpgt:Root%&path=/order/unapproved/show&isCache=true"
                }
                else
                {
                  location.reload();
