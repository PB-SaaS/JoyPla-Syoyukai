...
try {
    $action = new SendOrderBatchController(
        new GetAuthenticationInteractor(
            new MedicodeAuthenticationRepository(),
            new SPIRALAuthenticationRepository()
        ),
        new UpdateAccessTokenInteractor(
            new MedicodeAuthenticationRepository(),
            new SPIRALAuthenticationRepository()
        ),
    ...
    $errors[] = [
        ...
        date('Y-m-d H:i:s') .
        '.' .
        substr(explode('.', microtime(true) . '.000')[1], 0, 3),
        $ex->getCode(),
        $ex->getMessage() .
        "\n" .
        '[ ' .
