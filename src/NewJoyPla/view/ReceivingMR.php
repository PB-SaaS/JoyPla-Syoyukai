                            ...
                            <table class="uk-table uk-table-hover uk-table-middle uk-table-divider">
                                ...
                                <?php if ($report['count'] > 0) {
                                    foreach ($report['data'] as $record) {
                                        ...
                                        echo '<td>';
                                        foreach (
                                            $record['receivingCount']
                                            as $key => $receivingCount
                                        ) {
                                            echo $receivingCount .
                                                ' ' .
                                                $record['itemUnit'][$key] .
                                        ...
                                        echo '<td>';
                                        foreach (
                                            $record['totalReturnCount']
                                            as $key => $totalReturnCount
                                        ) {
                                            echo "<span class=\"uk-text-danger\">" .
                                                $totalReturnCount .
                                                ' ' .
                                                $record['itemUnit'][$key] .
                                                '</span><br>';
...
class ReceivingMR
{
    ...
    exportCSV(records)
    {
        ...
        for (let i = 1; i < records.length; i++) {
            for (let j = 0; j < records[i][9].length; j++) {
                ...
                remakeArray[k][17] = records[i][17][j];
            }
        }
    ...
    listDl()
    {
        ...
        result.unshift(['id', '院内商品ID', 'メーカー', '分類', '小分類', '商品名', '製品コード', '規格', 'JANコード', '卸業者名', '入数', '購買価格', '入庫数', '合計金額', '個数単位', '返品数', '調整額', '調整後金額']);
