        ...
        <div class="uk-container uk-container-expand">
            ...
            <form class="uk-form-stacked uk-child-width-1-2 uk-width-1-2@m" uk-grid>
                ...
                <div class="uk-width-1-1 uk-text-center">
                    <button onclick="payout_Label.setting()" type="button" class="uk-button uk-button-small ">反映</button>
                </div>
            ...
            foreach($payout_items as $item){
				...
				for($rnum = 1 ; $rnum <= $max; $rnum++){
                    ...
                    $payout_num = str_replace('payout_', '', $item->payoutId);
                    $barcodeId = "30".$payout_num; // 払出から生成できるバーコードは30にする。
                    ...
                    $design = str_replace('%JoyPla:sourceDivisionName%',$item->sourceDivision,				        $design);//払い出し元部署
                    $design = str_replace('%JoyPla:sourceRackName%',	($item->sourceRackName)? $item->sourceRackName : '(登録なし)', $design);//払い出し元部署棚
                    $design = str_replace('%JoyPla:divisionName%',		$item->targetDivision,					    $design);//払い出し先部署 
                    $design = str_replace('%JoyPla:rackName%',			($item->targetRackName)? $item->targetRackName : '(登録なし)',	$design);//払い出し先部署棚
...
<script>
    ...
    let payout_Label = new PayoutLabel();
