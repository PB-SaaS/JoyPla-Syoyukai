    ...
    <div class="uk-section uk-section-default uk-preserve-color uk-padding-remove" id="page_top">
        <div class="uk-container uk-container-expand uk-margin-top">
            ...
            <div class="uk-margin-bottom">
                <div>
                    <div>
                        <button class="uk-button uk-button-default" onclick="consume.sanshouClick()">商品マスタを開く</button>
                        <button class="uk-button uk-button-default" type="submit" onclick="window.print();return false;">印刷プレビュー</button>
                        <button class="uk-button uk-button-primary goodsBillingButton" onclick="consume.sendGoodsBilling()">消費登録</button>
                        <button class="uk-button uk-button-primary unorderedSlipButton" onclick="consume.sendUnorderedSlip(false)">未発注伝票作成</button>
                    </div>
                </div>
            </div>
            <div uk-sticky="sel-target: .uk-navbar-container; cls-active: uk-navbar-sticky" class="uk-padding-top uk-background-muted uk-padding-small">
                <form action='#' method="post" onsubmit="consume.barcodeSearch(); return false">
...
class ConsumeRegister
{
            ...
            //html = document.createTextNode(listObject[object.recordId].row);
                ...
                if (tmp.dataKey[i] === 'kakaku') {
                    ...
                    text += (tmp.useUnitPrice)
                      ...
                      : price_text( String(parseInt(tmp.listObject[tmp.index]['kakaku']) / parseInt(tmo.listObject[tmp.index]['irisu'])) );
    ...
    goodsBillingValidationCheck(){
        ...
        let chkLot = true;
        Object.keys(tmp.listObject).forEach(function (key) {
            if ((tmp.listObject[key]['lotFlag']) || (tmp.listObject[key]['countNum'] > 0)) {
    ...
    unorderedSlipValidationCheck(regData){
        if ($('select[name="busyo"]').val()) {
        } else {
            UIkit.modal.alert('部署を選択してください');
            return false ;
        ...
        let checkflg = false;
        Object.keys(regData).forEach(function (key) {
            if (Math.floor(regData[key].countNum / regData[key].irisu) > 0) {
                checkflg = true;
            }
        });
    ...
    sendUnorderedSlip(goodsFlg){
        ...
        UIkit.modal.confirm('未発注伝票を作成しますか？').then(function () {
            ...
            tmp.setRegData();
            let regOrdered = tmp.mergeItem();
            if (!tmp.unorderedSlipValidationCheck(regOrdered)) {
                loading_remove();
                return false;
            }
            ...
            $.ajax({
                ...
                data:{
                    ...
                    ordered : JSON.stringify( objectValueToURIencode(regOrdered) ),
    ...
    sendGoodsBilling(){
        ...
        UIkit.modal.confirm('消費登録を行いますか？').then(function () {
            ...
            loading();
            tmp.setRegData();
            if (!tmp.goodsBillingValidationCheck()) {
                loading_remove();
                tmp.canAjax = true; // 再びAjaxできるようにする
                return false;
            }
            let billing = tmp.mergeLot();
            $.ajax({
                async: false,
                url:'<?php echo $api_url ?>',
                type:'POST',
                data:{
                    ...
                    billing : JSON.stringify( objectValueToURIencode(billing) ),
                    divisionId : $('select[name="busyo"]').val()
                },
                dataType: 'json'
            })
    ...
    barcodeSearch(){
        ...
        .done( (data) => {
            ...
            if (data.lotNumber || data.lotDate) {
                tmp.addLotData(data,data.inHospitalItemId,data.lotNumber,data.lotDate);
            } else {
    ...
    gs1_128(gs1128){
        ...
        if (gs1128.indexOf("]C1") !== 0) {
            ...
            tmp.addLotData(setObj,objkey,objLot,objLotDate)
    ...
    mergeItem(){
        ...
        Object.keys(tmp.listObject).forEach(function (key) {
            let itemId = tmp.listObject[key]['recordId'];
            if (regObj[itemId] === void 0) { 
                regObj[itemId] = {};
                regObj[itemId] = tmp.listObject[key];
            } else {
                regObj[itemId]['countNum'] = regObj[itemId]['countNum'] + tmp.listObject[key].countNum;
            }
        });
        return regObj;
    }
...
let consume = new ConsumeRegister();
...
{
    consume.addTr(object, type, count);
}
</script>