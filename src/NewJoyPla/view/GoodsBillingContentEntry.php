...
<html>
  <head>
    ...
    <script>
		...
		function unorderedSlipValidationCheck(regData){
			if($('select[name="busyo"]').val()){
			} else {
				UIkit.modal.alert('部署を選択してください');
				return false ;
		...
		function sendUnorderedSlip(goodsFlg){
			...
			UIkit.modal.confirm('未発注伝票を作成しますか？').then(function () {
				...
				setRegData();
				let regOrdered = mergeItem();
				if(!unorderedSlipValidationCheck(regOrdered)){
					loading_remove();
					return false;
				}
		...
		function mergeItem(){
			...
			Object.keys(listObject).forEach(function (key) {
				...
				if (regObj[itemId] === void 0) { 
					regObj[itemId] = {};
					regObj[itemId] = listObject[key];
				} else {
					regObj[itemId]['countNum'] = regObj[itemId]['countNum'] + listObject[key].countNum;
				}
			});
			return regObj;
		}
