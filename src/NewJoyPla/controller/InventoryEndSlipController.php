...
class InventoryEndSlipController extends Controller
{
    ...
    public function index(): View
    {
        ...
        try {
            ...
            if ($end_slip->inventoryStatus != 2) {
                $history_items = Inventory::where(
                    'hospitalId',
                    $user_info->getHospitalId()
                )
                    ->where('inventoryEndId', $end_slip->inventoryEndId)
                    ->get();
                ...
                InventoryEnd::where('hospitalId', $user_info->getHospitalId())
                    ->find($record_id)
                    ->update([
                        'totalAmount' => (string) collect($prices)->sum(),
                        'itemsNumber' => (string) collect(
                            $in_hospital_item_ids
                        )->count(),
                    ]);
            ...
            $content = $this->view(
                'NewJoyPla/view/InventoryInformationByDivision',
                [
                    'api_url' => $api_url,
                    'user_info' => $user_info,
                    'link' => $link,
    ...
    public function slipFixApi(): View
    {
        ...
        try {
            ...
            $inventory_items = Inventory::where(
                'hospitalId',
                $user_info->getHospitalId()
            )
                ->where('inventoryEndId', $end_slip->inventoryEndId)
            ...
            foreach ($inventory_items->data->all() as $item) {
                if (
                    !in_array($item->inHospitalItemId, $in_hospital_items_ids)
                ) {
            ...
            foreach ($inventory_items->data->all() as $record) {
                if ($record->lotNumber && $record->lotDate) {
                    $lot_date = \App\Lib\changeDateFormat(
                        'Y年m月d日',
                        $record->lotDate,
                        'Y-m-d'
                    );
            ...
            foreach ($stock_items->data->all() as $stock_item) {
                ...
                $update_stock_data[$stock_item->inHospitalItemId] =
                    $update_stock_data[$stock_item->inHospitalItemId] +
                    (int) $stock_item->stockQuantity;
            }
            ...
            InHospitalItem::where(
                'hospitalId',
                $user_info->getHospitalId()
            )->bulkUpdate('inHospitalItemId', $in_hospital_update);
