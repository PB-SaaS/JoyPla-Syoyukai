    ...
    public function inventoryRegistAPI()
    {
        ...
        try {
            ...
            foreach ($inventory as $key => $record) {
                ...
                if (!$exist) {
                    throw new Exception('Inventory item does not belong to the hospital.', 999);
                }
                ...
                if ($record['lotDate']) {
                    ...
                    } elseif (preg_match(DateYearMonthDay::FORMAT_DELIMITER_HYPHEN, $record['lotDate'])) {
                        $inventory[$key]['lotDate'] = \App\Lib\changeDateFormat('Y-m-d', $record['lotDate'], 'Y-m-d');
                    } elseif (preg_match(DateYearMonthDay::FORMAT_DELIMITER_JAPANESE_CHARACTER, $record['lotDate'])) {
