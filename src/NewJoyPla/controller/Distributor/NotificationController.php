...
class NotificationController extends Controller
{
    public $links = [
        ...
        'undoneOrder' => '%url/rel:mpgt:OrderD%'
        ];
    ...
    public $icons = [
        ...
        'undoneOrder' => "https://i02.smp.ne.jp/u/joypla/images/menu_icon/6.png"
    ];
    ...
    public function index(): View
    {
        ...
        try {
            ...
            if ($user_info->isHospitalUser()) {
                ...
                if ($user_info->isUser()) {
                    $instance = $instance->where('divisionId', $user_info->getDivisionId());
                }
            ...
            if ($user_info->isDistributorUser()) {
                $distributorAffiliations = DistributorAffiliationView::where('loginId', $user_info->getLoginId())->where('invitingAgree', '1')->get();
                $distributorAffiliations = $distributorAffiliations->data->all();

                $instance = OrderHistory::where('orderStatus', '1', '>')->where('orderStatus', '6', '<');

                foreach ($distributorAffiliations as $affiliation) {
                    $instance->orWhere('distributorId', $affiliation->distributorId);
                }

                $count = $instance->count();
                if ($count > 0) {
                    $notification[] = [
                        'message' => '納品未完了伝票が<br><span class="uk-text-danger uk-text-bold">'.$count.'件</span>存在します',
                        'icon' => $this->icons['undoneOrder'],
                        'link' => $this->links['undoneOrder'],
                    ];
                }
            }
