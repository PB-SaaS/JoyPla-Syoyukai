...
class ReceiptSlipByItemController extends Controller
{
    ...
    public function acceptanceDocument(): View
    {
        ...
        try {
            ...
            $receiving_item = Receiving::where('hospitalId', $user_info->getHospitalId())->find($record_id)->get();
            $receiving_item = $receiving_item->data->get(0);

            $receiving_history = ReceivingHistoryView::where('hospitalId', $user_info->getHospitalId())->where('receivingHId', $receiving_item->receivingHId )->get();
 ...
 * 実行
 */
$ReceiptSlipByItemController = new ReceiptSlipByItemController();
...

{
    ...
    {
        echo $ReceiptSlipByItemController->returnSlipEntry()->render();
    } 
    else if($action === "regReturnApi")
    {
        echo $ReceiptSlipByItemController->regReturnApi()->render();
    }
    else 
    {
        echo $ReceiptSlipByItemController->acceptanceDocument()->render();
    }
}
