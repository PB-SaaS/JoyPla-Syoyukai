...
class UsedSlipController extends Controller
{
        ...
        try {
            ...
            foreach ($borrowing as $borrowing_key => $data) {
                foreach ($in_Hospital_item->data->all() as $item) {
                    if ($item->inHospitalItemId === $data->inHospitalItemId) {
                        $borrowing[$borrowing_key]->makerName =
                            $item->makerName;
                        $borrowing[$borrowing_key]->itemName = $item->itemName;
                        $borrowing[$borrowing_key]->itemCode = $item->itemCode;
                        $borrowing[$borrowing_key]->itemStandard =
                            $item->itemStandard;
                        $borrowing[$borrowing_key]->itemJANCode =
                            $item->itemJANCode;
                        break;
                    }
                }
            ...
            if ($used_slip_history->usedSlipStatus == '2') {
                ...
                if ($user_info->isUser()) {
                    if (
                        preg_match(
                            '/Action=approvedUsedSlipDivision/',
                ...
                if ($user_info->isHospitalUser()) {
                    $association = AssociationTR::where(
                        'usedSlipId',
                        $used_slip_history->usedSlipId
                    )->get();
                ...
                if ($user_info->isUser()) {
                    if (
                        preg_match(
                            '/Action=unapprovedUsedSlipDivision/',
                            $_SERVER['HTTP_REFERER']
                        )
                    ) {
                        $box = parse_url($_SERVER['HTTP_REFERER']);
    ...
    public function usedSlipApprovalApi()
    {
        ...
        try {
            ...
            if ($user_info->isDistributorUser()) {
                throw new Exception(
                    FactoryApiErrorCode::factory(203)->getMessage(),
                    FactoryApiErrorCode::factory(203)->getCode()
                );
            }

            $used_slip_history = UsedSlipHistoy::find($record_id)
            ...
            if ($used_slip_history->usedSlipStatus != 1) {
                throw new Exception(
                    FactoryApiErrorCode::factory(900)->getMessage(),
                    FactoryApiErrorCode::factory(900)->getCode()
                );
            }
            ...
            foreach ($used_slip_create_data['ids'] as $history) {
                $mail_body = $this->view(
                    'NewJoyPla/view/Mail/UsingRegistration',
                    [
                        'name' => '%val:usr:name%',
                        ...
                        'total_price' =>
                            '￥' .
                            number_format_jp(
                                (float) $used_slip_history->totalAmount,
                                2
                            ),
                ...
                $test = DistributorAffiliationView::selectName($select_name)
                    ->rule([
                        'name' => 'distributorId',
                        'label' => 'name_' . $used_slip_history->distributorId,
                        'value1' => $used_slip_history->distributorId,
                        'condition' => 'matches',
                    ])
                    ->rule([
                        'name' => 'invitingAgree',
            ...
            $result = UsedSlipHistoy::find($record_id)
                ->where('hospitalId', $user_info->getHospitalId())
                ->update([
                    'usedSlipStatus' => 2,
                ]);
    ...
    private function usedReportApi(array $borrowing_data)
    {
        ...
        if ($this->in_hospital_items == null) {
            ...
            foreach ($borrowing_data as $data) {
                $in_hospital_items_instance = $in_hospital_items_instance::orWhere(
                    'inHospitalItemId',
        ...
        foreach ($borrowing_data as $borrowing_key => $data) {
            ...
            foreach ($this->in_hospital_items->data->all() as $key => $val) {
                if ($val->inHospitalItemId == $data->inHospitalItemId) {
                    $used_report[$borrowing_key]->inHospitalItemId =
                        $val->inHospitalItemId;
                    $used_report[$borrowing_key]->hospitalId = $val->hospitalId;
                    $used_report[$borrowing_key]->distributorId =
                        $val->distributorId;
                    ...
                    $used_report[$borrowing_key]->quantityUnit =
                        $val->quantityUnit;
                    $used_report[$borrowing_key]->itemUnit = $val->itemUnit;
                    $used_report[$borrowing_key]->notUsedFlag =
                        $val->notUsedFlag;
                    ...
                    $used_report[$borrowing_key]->itemStandard =
                        $val->itemStandard;
                    $used_report[$borrowing_key]->itemJANCode =
                        $val->itemJANCode;
        ...
        foreach ($used_report as $item) {
            if (
                ...
                !$history_ids[
                    $item->divisionId . $item->distributorId . $item->usedDate
                ]
            ) {
        ...
        foreach ($used_report as $item) {
            ...
            $order_insert_data[$item->inHospitalItemId]['orderQuantity'] =
                $order_insert_data[$item->inHospitalItemId]['orderQuantity'] +
                (float) $item->countNum;
            $order_insert_data[$item->inHospitalItemId]['orderPrice'] =
                $order_insert_data[$item->inHospitalItemId]['orderPrice'] +
                (float) $item->price * (float) $item->countNum;
            $order_insert_data[$item->inHospitalItemId]['receivingNum'] =
                $order_insert_data[$item->inHospitalItemId]['receivingNum'] +
                (float) $item->countNum;
        }
        ...
        foreach (
            $history_ids
            as $divisionId_distributorId_usedDate => $history_data
        ) {
            $order_price = [];
            $in_hospital_item_ids = [];
            foreach ($order_insert_data as $insert_record) {
                if (
                    $insert_record['orderNumber'] ===
                    $history_data['orderHistoryId']
                ) {
                    $order_price[] = (float) $insert_record['orderPrice'];
                    if (
                        !in_array(
                            $insert_record['inHospitalItemId'],
                            $in_hospital_item_ids
                        )
                    ) {
                        $in_hospital_item_ids[] =
        ...
        foreach ($used_report as $item) {
            foreach ($order_insert_data as $order_data) {
                $orderCNumber = '';
                if (
                    $item->inHospitalItemId == $order_data['inHospitalItemId']
                ) {
        ...
        foreach (
            ...
            foreach ($receiving_insert_data as $insert_record) {
                if (
                    $insert_record['receivingHId'] ===
                    $history_data['receivingHistoryId']
                ) {
                    $receiving_price[] =
                        (float) $insert_record['receivingPrice'];
                    if (
                        !in_array(
                            $insert_record['inHospitalItemId'],
                            $in_hospital_item_ids
        ...
        foreach ($used_report as $item) {
            ...
            $billing_insert_data[] = [
                ...
                'price' => (float) $item->price,
                'billingQuantity' =>
                    (float) $item->quantity * (float) $item->countNum,
                'billingAmount' =>
                    $unitPrice *
                    ((float) $item->quantity * (float) $item->countNum),
                'hospitalId' => $item->hospitalId,
        ...
        foreach (
            ...
            foreach ($billing_insert_data as $insert_record) {
                if (
                    $insert_record['billingNumber'] ===
                    $history_data['billingHistoryId']
                ) {
    ...
    private function association(array $used_slip_ids, array $other_ids)
    {
        ...
        foreach (
            $used_slip_ids
            as $divisionId_distributorId_usedDate => $data
        ) {
            $insert_data[] = [
                'usedSlipId' =>
                    $used_slip_ids[$divisionId_distributorId_usedDate][
                        'usedSlipId'
                    ],
                'orderNumber' =>
                    $other_ids[$divisionId_distributorId_usedDate][
                        'orderHistoryId'
                    ],
                'receivingHId' =>
                    $other_ids[$divisionId_distributorId_usedDate][
