...
class RegistRequestItemController extends Controller
{
    ...
    public function input(): View
    {
        global $SPIRAL;
        try {
            $content = $this->view('NewJoyPla/view/Form/ReqItemsReg/Input', [
    ...
    public function confirm(): View
    {
        global $SPIRAL;
        try {
            $content = $this->view('NewJoyPla/view/Form/ReqItemsReg/Confirm', [
                    'csrf_token' => Csrf::generate(16),
            ...
            $head = $this->view('NewJoyPla/view/template/parts/Head', [] , false);
            // テンプレートにパラメータを渡し、HTMLを生成し返却X
            return $this->view('NewJoyPla/view/template/Template', [
                'title'     => 'JoyPla 見積商品追加 - 確認',
                'content'   => $content->form_render(),
                'head' => $head->render(),
                'header' => '',
                'baseUrl' => '',
    ...
    public function thank(): View
    {
         ...
         *  7	採用
            ...
            $subject = "[JoyPla] 見積商品が登録されました";
            
            $mail_body = $this->view('NewJoyPla/view/Mail/RegistRequestItem', [
                'name' => '%val:usr:name%',
