    ...
    public function index(): View
    {
        ...
        try {
			...
			if(preg_match('/^20/', $barcode) && strlen($barcode) == 12){
				...
				$in_hospital_item = InHospitalItem::where('inHospitalItemId',$record->inHospitalItemId)->where('hospitalId', $user_info->getHospitalId())->get();	
				...
				$result = InHospitalItem::where('labelId',$label_id)->where('hospitalId', $user_info->getHospitalId())->get();
				if($result->count == '0'){
					throw new Exception(FactoryApiErrorCode::factory(191)->getMessage(),FactoryApiErrorCode::factory(191)->getCode());
				}
				$record = $result->data->get(0);
