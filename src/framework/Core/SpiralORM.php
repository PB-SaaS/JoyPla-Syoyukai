...
class SpiralORM
{
    ...
    public function __construct()
    {
        ...
        $this->spiralDataBase = new SpiralDataBase(
            $SPIRAL,
            $spiralApiCommunicator,
            $spiralApiRequest
        );
        if (
            isset($this::$mail_field_title) &&
            $this::$mail_field_title != null
        ) {
            $this->spiralSendMail = new SpiralSendMail(
                $SPIRAL,
                $spiralApiCommunicator,
                $spiralApiRequest
            );
            $this->spiralDBFilter = new SpiralDBFilter(
                $SPIRAL,
                $spiralApiCommunicator,
                $spiralApiRequest
            );
    ...
    public static function whereDeleted()
    {
        ...
        if ($instance::DELETED_AT !== '') {
            $instance
                ->orWhere($instance::DELETED_AT, '1')
                ->orWhere($instance::DELETED_AT, '0')
                ->orWhere($instance::DELETED_AT, '0', 'ISNULL');
    ...
    public static function get()
    {
        ...
        foreach ($instance->sort as $sort) {
            $instance->spiralDataBase->addSortField(
                $sort['name'],
                $sort['order']
            );
        }
        ...
        if ($result['label']) {
            $result['label'] = $instance->spiralDataBase->labelToNameArray(
                $result['label'],
                $column
            );
        }
        if ($result['count'] > 0) {
            $result['data'] = $instance->spiralDataBase->arrayToNameArray(
                $result['data'],
                $column
            );
            if (
                $instance->select_fields === null &&
                $instance->plain === null
            ) {
        ...
        if ($result['code'] != 0) {
            throw new Exception(
                FactoryApiErrorCode::factory(
                    (int) $result['code']
                )->getMessage(),
                FactoryApiErrorCode::factory((int) $result['code'])->getCode()
            );
        }
    ...
    public static function sort(string $fieldTitle, string $order = 'asc')
    {
        $instance = self::getInstance();
        $column = array_merge($instance::$fillable, $instance::$guarded);
        if (array_search($fieldTitle, $column, true) === false) {
            $fieldTitle = 'id';
            $order = 'asc';
        }
        if ($order == 'asc' || $order == 'desc') {
            $instance->sort[] = ['name' => $fieldTitle, 'order' => $order];
        }
        return $instance;
    }
    ...
    public static function bulkUpdate(string $key, array $update_data)
    {
        ...
        $result = $instance->spiralDataBase->doBulkUpdate(
            $key,
            $fileds,
            $update
        );
    ...
    private function makeBulkUpdateArray(array $update_data)
    {
        ...
        if (
            static::UPDATED_AT != '' &&
            in_array(static::UPDATED_AT, $fieldnames) === false
        ) {
    ...
    public static function upsert($key, array $upsert_data)
    {
        ...
        */
        $result = $instance->spiralDataBase->doBulkUpsert(
            $key,
            $fields,
            $upsert
        );
    ...
    private function makeInsertArray(array $insert_data)
    {
        ...
        if (
            static::CREATED_AT != '' &&
            in_array(static::CREATED_AT, $fieldnames) === false
        ) {
    ...
    public function save()
    {
        ...
        $this->spiralDataBase->addSearchCondition(
            $this::$primary_key,
            $this->{$this::$primary_key}
        );
        $save_data = $this->makeSaveArray();
        $result = $this->spiralDataBase->doUpsert(
            $this::$primary_key,
            $save_data
        );
        if ($result['code'] != 0) {
    ...
    public static function send()
    {
        ...
        $instance->spiralSendMail->addMailFieldTitle(
            $instance::$mail_field_title
        );
