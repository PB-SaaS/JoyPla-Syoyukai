...
class SiDateTime extends DateTime
{
    ...
    public function __construct($date, $timezone = '')
    {
        ...
        else if(self::hasFormat($date , 'Y年m月d日 H時i分s秒'))
        {
            $date = parent::createFromFormat('Y年m月d日 H時i分s秒', $date , $zone);
            $date = $date->format('Y-m-d H:i:s.u');
        }
    ...
    public static function setHolidayConfig($file = 'framework/Library/SiDateTime/HolidayConfig.php')
    {
    ...
    {
        return ($old->getTimestamp() <= $this->getTimestamp() && $this->getTimestamp() <= $latest->getTimestamp());
    }
    ...
    {
        return 1 + date('W', $this->getTimestamp() + 86400) - date('W', strtotime(date('Y-m', $this->getTimestamp())) + 86400);
    }
    ...
    public function __set($name, $value)
    {
        switch ($name) 
        {
            ...
            case 'hour' : {
                $this->setTime($value, $this->minute, $this->second, $this->micro);
                break;
            }
            case 'minute' : {
                $this->setTime($this->hour,  $value, $this->second, $this->micro);
                break;
            }
            case 'second' : {
                $this->setTime($this->hour, $this->minute, $value, $this->micro);
                break;
            }
            case 'micro' : {
                $this->setTime($this->hour, $this->minute, $this->second, $value);
                break;
            }
            case 'timestamp' : {
