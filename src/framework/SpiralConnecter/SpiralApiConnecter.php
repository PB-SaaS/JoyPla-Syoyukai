...
class SpiralApiConnecter implements SpiralConnecterInterface  {
    ...
    public function request(XSpiralApiHeaderObject $header, HttpRequestParameter $httpRequestParameter)
    {
        ...
        $httpRequestParameter->set('signature' , hash_hmac('sha1', $key, $this->secret, false));
    ...
    public function bulkRequest(XSpiralApiHeaderObject $header, array $httpRequestParameters)
    {
        $request = new HttpRequest();
        ...
        foreach($responses as $key => $response)
        {
            ...
            $logs[] = $this->logObject($header , $httpRequestParameters[$key] ,$response);
    ...
    private function logObject($header , HttpRequestParameter $request , $response)
    {
        return [
