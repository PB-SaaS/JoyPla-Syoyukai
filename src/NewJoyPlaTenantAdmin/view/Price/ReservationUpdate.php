<div class="uk-section uk-section-default uk-preserve-color uk-padding-remove" id="app">
	<div class="uk-container uk-container-expand" uk-height-viewport="expand: true">
		<div class="uk-margin-auto uk-margin-remove-top uk-margin-bottom" id="mainPage">
			...
			<input type="button" v-on:click="validateCheck" value="バリデーションチェック" class="uk-button uk-button-primary"
				v-bind:disabled="lists.length == 0">
				...
				<tbody>
					<tr v-for="(worker, index) in lists" :key="index">
						...
						<td><button type="button" class="uk-button uk-button-danger uk-button-small"
								v-on:click="deleteList(index)">削除</button></td>
					</tr>
				</tbody>
				<tfoot>
					<tr></tr>
	...
	var app = new Vue({
		...
		methods: {
			...
			loadCsvFile(e) {
				...
				if (!file.type.match("text/csv") && !file.type.match("application/vnd.ms-excel") && (extension !=
						'tsv' && extension != 'txt')) {
				...
				for (i; i < chunk.length; i++) {
					...
					promises.push($.ajax({
						...
						data: {
							...
							Action: "reservationUpdateValidateCheckApi",
						},
						dataType: "json"
					}).always( (data) => {
						let percent = 100;
					...
					promises.push($.ajax({
						...
						data: {
							...
							rowData: JSON.stringify(vm.objectValueToURIencode(chunk[i])),
							_csrf: "<?php echo $csrf_token; ?>", // CSRFトークンを送信
							Action: "reservationUpdateValidateCheck2Api",
						},
						dataType: "json"
					}).always( (data) => {
						let percent = 100;
				...
				UIkit.modal.confirm("登録処理を行います。<br>よろしいでしょうか。").then(function() {
					...
					for (i; i < chunk.length; i++) {
						...
						promises.push($.ajax({
							...
							data: {
								startRowNumber: startRowNumber,
								rowData: JSON.stringify(vm.objectValueToURIencode(
									chunk[i])),
								_csrf: "<?php echo $csrf_token; ?>", // CSRFトークンを送信
								Action: "reservationUpdateApi",
							},
							dataType: "json"
						}).always( (data) => {
							let percent = 100;
